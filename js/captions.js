...
// Language-aware hover captions for gallery
const CAPTIONS={
...
6:{hr:"Lagana konstrukcija: manje opterećenje na zidove.",en:"Lightweight: less load on walls and structure.",uk:"Легкість: менше навантаження на конструкції будівлі."},
...
10:{hr:"Staklo (kaljeno/triplex): otporno i dugotrajno.",en:"Tempered/laminated glass: durable and robust.",uk:"Загартоване/триплекс скло: міцне і довговічне."},
...
12:{hr:"LED rasvjeta po želji za efektan izgled.",en:"Optional LED lighting for a striking effect.",uk:"Можлива LED‑підсвітка для ефектного вигляду."}
};

function currentLang(){
  const p=new URLSearchParams(location.search).get('lang');
  return p || localStorage.getItem('site_lang') || 'hr';
}

function applyCaptions(lang=currentLang()){
  document.querySelectorAll('.gallery .g').forEach(fig=>{
    const id=fig.getAttribute('data-id');
    const t=(CAPTIONS[id]&&(CAPTIONS[id][lang]||CAPTIONS[id].hr))||'';
    const img=fig.querySelector('img'); if(img) img.alt=t;
    const fc=fig.querySelector('figcaption'); if(fc) fc.textContent=t;
    const oc=fig.querySelector('.overlay-cap'); if(oc) oc.textContent=t;
  });
}

document.addEventListener('DOMContentLoaded',()=>{
  document.body.classList.add('captions-hover');
  applyCaptions();
  const sel=document.getElementById('lang');
  if(sel){ sel.addEventListener('change',()=>applyCaptions(sel.value)); }
  window.addEventListener('popstate',()=>applyCaptions());
});
